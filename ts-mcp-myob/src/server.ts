...
const SPEC_PATH =
  process.env.MYOB_OPENAPI_PATH ||
  // at runtime this file is in dist/, so go up to project root
...
async function main() {
  ...
  // Backend HTTP client with cookie jar
  ...
  // Generate tools from OpenAPI unless auth-only mode
  let toolCount = 0;
  if (mode !== 'auth') {
    try {
      const spec = loadOpenAPISpec(SPEC_PATH);
      const tools = buildToolsFromOpenAPI(spec, httpClient, BASE_URL);
      // Use a permissive input schema (raw shape) so clients can pass common args
  ...
  // Legacy lightweight helpers removed per request

  // Streamable HTTP transport
  const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: undefined, enableJsonResponse: true });
  await mcp.connect(transport);

  // Minimal web server with CORS and OPTIONS
