...
// Augment AxiosInstance with cookie jar helpers we manage manually
export interface CookieAxios extends AxiosInstance {
  __cookieJar?: any;
  __cookieHeader?: string; // cached Cookie header string after login
}

// Build absolute URL from axios config using baseURL
...
export function createHttpClient(baseURL: string): CookieAxios {
  ...
  // Attach jar and intercept requests to add Cookie header
  ...
  client.interceptors.request.use(async (config) => {
    try {
      ...
      // Fallback: aggregate cookies across all paths for this origin if none matched by path
      if (!cookieStr) {
        try {
    ...
    // Always keep credentials enabled
    (config as any).withCredentials = true;
    return config;
  });

  // Capture Set-Cookie from all responses into the jar
...
async function getCookiesForOrigin(http: CookieAxios, origin: string) {
  ...
  // Ensure a valid absolute URL (with trailing slash) for tough-cookie
  let originUrl = origin;
  try {
    const o = new URL(origin);
    originUrl = new URL('/', o.origin).toString();
  } catch {
    try {
...
export function registerAuthTools(mcp: McpServer, http: CookieAxios, loginUrl: string, logoutUrl: string) {
  ...
  const redactObj = (o: Record<string, any>) => {
    const out: Record<string, any> = {};
    if (!o) return out;
    for (const [k, v] of Object.entries(o)) out[k] = redactHdr(k) ? '[REDACTED]' : v;
    return out;
  };

  // Login tool (aligns with server.py shape)
  const LoginInput = {
  ...
  mcp.registerTool(
    'login',
    { description: 'Login to MYOB (cookie session). Required before calling other tools.', inputSchema: LoginInput },
    (async (args: unknown) => {
      // Normalize incoming args from various client shapes
      let a: any = args;
      try {
        if (typeof a === 'string') {
          try {
        ...
        if (a && typeof a === 'object') {
          if ((a as any).arguments && typeof (a as any).arguments === 'object') a = (a as any).arguments;
          if ((a as any).input && typeof (a as any).input === 'object') a = (a as any).input;
          if (Array.isArray((a as any).content)) {
            const jsonPart = (a as any).content.find((c: any) => c && c.type === 'json' && c.json);
            const textPart = (a as any).content.find((c: any) => c && c.type === 'text' && typeof c.text === 'string');
      ...
      if (DEBUG_HTTP) {
        try {
          ...
          console.log('[MYOB TS MCP][LOGIN ARGS]', JSON.stringify(a).slice(0, 1500));
        } catch {}
      }

      // Accept both name/username/user/email keys; fallback to env if args missing
      const envJson = process.env.MYOB_AUTOLOGIN_JSON as string | undefined;
      let envCreds: any = {};
      try {
        envCreds = envJson ? JSON.parse(envJson) : {};
      } catch {}

      const payload = {
        name:
          a?.name ?? a?.username ?? a?.user ?? a?.email ?? envCreds?.name ?? envCreds?.username ?? envCreds?.user ?? envCreds?.email ?? process.env.MYOB_LOGIN_NAME,
        password: a?.password ?? envCreds?.password ?? process.env.MYOB_LOGIN_PASSWORD,
        company: a?.company ?? envCreds?.company ?? process.env.MYOB_LOGIN_COMPANY,
        branch: a?.branch ?? envCreds?.branch ?? process.env.MYOB_LOGIN_BRANCH,
      };
      ...
      try {
        // Preflight: hit login origin to seed initial cookies if required by server
        try {
          const preUrl = new URL('/', new URL(loginUrl).origin).toString();
          if (DEBUG_HTTP) console.log('[MYOB TS MCP][LOGIN OUT]', 'GET', preUrl);
          const pre = await http.get(preUrl, { withCredentials: true });
          if (DEBUG_HTTP) console.log('[MYOB TS MCP][LOGIN IN]', (pre as any)?.status, preUrl);
        } catch {}
        ...
        // Persist Set-Cookie into jar (safety)
        try {
          const setCookie = (resp as any)?.headers?.['set-cookie'] as string[] | undefined;
          if (setCookie && setCookie.length) {
            ...
            await Promise.all(
              setCookie.map(
                (sc) =>
                  new Promise<void>((resolve) => {
                    try {
                      jar.setCookie(sc, originAbs, () => resolve());
                    } catch {
                      resolve();
                    }
                  })
            ...
            // Also set on API origin (defense in depth)
            await Promise.all(
              setCookie.map(
                (sc) =>
                  new Promise<void>((resolve) => {
                    try {
                      jar.setCookie(sc, apiOriginAbs, () => resolve());
                    } catch {
                      resolve();
                    }
                  })
            ...
            // Cache a cookie header string for fallback usage
            try {
              const list = await new Promise<any[]>((resolve) => {
                try {
                  jar.getCookies(apiOriginAbs, { allPaths: true }, (err: any, cookies: any) => resolve(err ? [] : (cookies || [])));
                } catch {
                  resolve([]);
                }
              });
              ...
              if (hdr) {
                ...
                // Also set axios defaults for this client to force Cookie header on subsequent requests
                try {
              ...
              if (DEBUG_HTTP) {
                ...
                console.log('[MYOB TS MCP][LOGIN COOKIES]', {
                  loginOrigin,
                  loginCookies,
                  apiOrigin,
                  apiCookies,
                  defaultsCookie: (http as any).defaults?.headers?.common?.['Cookie'] ? '[SET]' : '[EMPTY]',
                });
              }
            } catch {}
          }
        ...
        // Return similar shape to server.py
        let data: any;
        try {
        ...
        return {
          content: [{ type: 'text' as const, text: 'login ok' }],
          structuredContent: { status_code: (resp as any).status, ok: (resp as any).status >= 200 && (resp as any).status < 300, data, origin, cookies },
        };
      } catch (err: any) {
        if (DEBUG_HTTP) console.warn('[MYOB TS MCP][LOGIN ERR]', err?.response?.status ?? '-', err?.message);
        const status = err?.response?.status ?? 500;
        const data = err?.response?.data ?? err?.message ?? 'Login failed';
        return { content: [{ type: 'text' as const, text: 'login failed' }], structuredContent: { status_code: status, ok: false, data } };
      }
    }) as any
  );

  // Fallback tool to accept raw JSON/text credentials from chat without schema issues
  mcp.registerTool(
    'login_raw',
    { description: 'Login using a raw JSON string or object. Fields: name,password,company,branch.' },
    (async (raw: any) => {
      try {
        let payload: any = raw;
        ...
        if (!body.name || !body.password || !body.company || !body.branch) {
          return { content: [{ type: 'text' as const, text: 'Missing fields. Provide JSON with name,password,company,branch.' }] };
        }
        ...
        return { content: [{ type: 'text' as const, text: `Login failed: ${e?.message || 'error'}` }] };
      }
    }) as any
  );

  // Logout tool (clears cookie jar)
  mcp.registerTool(
    'logout',
    { description: 'Logout current MYOB session and clear cookies.' },
    (async () => {
      try {
        ...
        try {
          const jar = http.__cookieJar as any;
          if ((jar as any)?.removeAllCookies)
            await new Promise<void>((resolve, reject) => (jar as any).removeAllCookies((e: any) => (e ? reject(e) : resolve())));
        } catch {}
        ...
        const status = err?.response?.status ?? 500;
        const data = err?.response?.data ?? err?.message ?? 'Logout failed';
        return { content: [{ type: 'text' as const, text: 'logout failed' }], structuredContent: { status_code: status, ok: false, data } };
      }
    }) as any
  );
}

// Optional autologin helper for startup usage (mirrors server.py behavior)
export async function tryAutoLogin(http: CookieAxios, loginUrl: string) {
  try {
    ...
    // Persist Set-Cookie manually (safety)
    try {
      const setCookie = (resp as any)?.headers?.['set-cookie'] as string[] | undefined;
      if (setCookie && setCookie.length) {
        const originAbs = new URL('/', new URL(loginUrl).origin).toString();
