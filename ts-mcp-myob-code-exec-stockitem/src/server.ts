...
function resolveSpecPath() {
  ...
  const candidates = [
    // Most common on Azure App Service when zipped: file alongside dist/ under wwwroot
    path.resolve(__dirname, '..', 'REDBACK_TEST_API_short.json'),
    // Fallback two levels up (older packaging)
    path.resolve(__dirname, '..', '..', 'REDBACK_TEST_API_short.json'),
    // repo root fallback (one level above project root)
    path.resolve(__dirname, '..', '..', '..', 'REDBACK_TEST_API_short.json'),
    // process cwd (some hosts set CWD to wwwroot)
    path.resolve(process.cwd(), 'REDBACK_TEST_API_short.json'),
    // sibling original package copy (dev)
